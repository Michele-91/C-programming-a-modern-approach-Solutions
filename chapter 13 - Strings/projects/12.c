...
in a two-dimensional char array as it reads the sentence, with each row of
the array storing a single word. Assume that the sentence contains no more
than 30 words and no word is more than 20 characters long. 
Be sure to store a null character at the end of each word so that it can 
...
void get_sentence(char sentence[MAX_WORDS][WORD_LEN+1], char *terminator);
void print_reverse_sentence(char sentence[MAX_WORDS][WORD_LEN+1], char terminator);
